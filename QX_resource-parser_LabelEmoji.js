      ...
      },
      {
        ...
        items: [
          { type: "tags", key: "in", label: "✅ 保留（in）",
            description: "按节点名关键字保留。每行一个关键字表示“或”；同行用 . 分隔表示“与”。例：另起一行填 香港、中国台湾 表示含其一即可；同一行填 香港.IPLC 表示同时含香港和 IPLC。",
          ...
          { type: "text", key: "regex", label: "🔎 正则保留（regex）",
            description: "对节点完整信息正则匹配以保留节点。匹配范围包括协议、端口、加密、host 等字段。",
            placeholder: "iplc" },
          { type: "text", key: "regout", label: "🚫 正则删除（regout）",
            description: "对节点完整信息正则匹配以删除节点。",
            placeholder: "" },
          { type: "text", key: "sort", label: "↕️ 节点排序（sort）",
            description: "对节点进行排序。参数为 1 正序、-1 逆序、x 随机排序，或使用 > / < 连接自定义规则。",
            placeholder: "🇭🇰>🇯🇵>🇨🇳>🇺🇸" }
        ]
      },
      {
        ...
        items: [
          { type: "select", key: "emoji", label: "🎌 Emoji 旗帜",
            description: "添加或删除节点名中的地区旗帜。中国台湾相关关键词统一使用 🇨🇳。",
            options: _emojiOptions() },
          { type: "text", key: "replace", label: "🔁 正则替换节点信息（replace）",
            description: "替换节点完整信息。可匹配所有内容，格式为 regex1@str1+regex2@str2。",
            placeholder: "" },
          { type: "switch", key: "udp", label: "📡 UDP Relay",
            onValue: "1", offValue: "-1",
            description: "强制开启或关闭 udp-relay。" },
          { type: "switch", key: "tfo", label: "⚡ Fast Open",
            onValue: "1", offValue: "-1",
            description: "强制开启或关闭 fast-open。" },
          { type: "switch", key: "cert", label: "🔐 TLS 证书验证",
            description: "开启或关闭 TLS 证书验证。默认关闭。",
            onValue: "1", offValue: "-1" },
          { type: "switch", key: "uot", label: "🧩 UDP over TCP",
            onValue: "1", offValue: "",
            description: "开启 udp-over-tcp=true。仅限 SS/SSR。" },
          { type: "switch", key: "aead", label: "🧬 VMess AEAD",
            description: "关闭 VMess AEAD 参数。",
            onValue: "", offValue: "-1" },
          { type: "text", key: "alpn", label: "🧷 ALPN",
            description: "指定 over-tls 节点的 ALPN 参数。",
            placeholder: "h2" },
          { type: "text", key: "host", label: "🌐 Host",
            description: "修改已有 host；如果需要新增 host，请用 ☠️ 结尾。",
            placeholder: "" },
          { type: "text", key: "checkurl", label: "🧪 Check URL",
            description: "指定 server_check_url 参数。",
      ...
      },
      {
        ...
        items: [
          ...
          { type: "switch", key: "clean", label: "🧹 删除伪节点",
            onValue: "1", offValue: "",
            description: "删除订阅说明、流量、官网、群组、更新、过期提示等伪节点。" },
          { type: "switch", key: "cleantag", label: "🧼 清理节点名特殊符号",
            onValue: "1", offValue: "",
            description: "只保留常见中英文、数字、空格和少量安全符号。" },
          { type: "switch", key: "autoe", label: "🎌 自动地区 Emoji",
            onValue: "1", offValue: "",
            description: "根据节点名自动识别地区并添加 emoji。中国台湾相关关键词统一使用 🇨🇳。" },
          { type: "switch", key: "airport", label: "🏷 插入机场名前缀",
            onValue: "1", offValue: "",
            description: "使用 QX 资源标签作为 [机场名]，优先插入到地区 emoji 后面。" },
          { type: "switch", key: "rsort", label: "🧭 按地区顺序排序",
            onValue: "1", offValue: "",
            description: "默认顺序：香港、日本、新加坡、中国、美国，其余节点沉底。" }
        ]
      },
      {
        type: "group",
        ...
        items: [
          ...
          { type: "switch", key: "info", label: "📊 流量信息",
            onValue: "1", offValue: "",
            description: "以通知形式推送订阅用量、剩余流量和到期时间等信息。" },
          { type: "text", key: "flow", label: "📈 流量参数",
            description: "格式：到期时间:总流量GB:已用GB，例如 2026-12-31:1000:54。",
            placeholder: "2026-12-31:1000:54" },
          { type: "select", key: "type", label: "🧱 强制类型",
            description: "当解析器未能正确识别类型时，可手动指定资源类型。",
            options: [
              { label: "自动", value: "" },
      ...
      },
      {
        ...
        items: [
          { type: "text", key: "via", label: "🔌 via-interface（规则订阅）",
            description: "0 表示 via-interface=%TUN%。设置代理链时，规则分流需使用此参数，并在策略偏好将策略组指定为落地策略组/节点。",
            placeholder: "填写 0 即可" },
          { type: "text", key: "relay", label: "🔗 代理链 Relay（节点订阅）",
            description: "将落地节点订阅转换为 ip/host 规则，指向中转策略组/节点。设置 Relay 时，请关闭该订阅首页的策略偏好设置。via 与 relay 不应在同一条引用中同时设置。",
      ...
      },
      {
        ...
        items: [
          ...
          { type: "switch", key: "cdn", label: "🚀 GitHub CDN 加速",
            onValue: "1", offValue: "",
            description: "将 GitHub 脚本地址转换为 CDN 链接。" },
          { type: "select", key: "dst", label: "🔄 转换目标（dst）",
            description: "指定转换目标为 rewrite 或 filter。",
            options: [
              { label: "默认（不转换）", value: "" },
          ...
          { type: "select", key: "fcr", label: "📶 网络接口指定（fcr）",
            description: "为分流规则添加 force-cellular、multi-interface 或 multi-interface-balance 参数。",
            options: [
              { label: "默认", value: "" },
    ...
    sections: [
      {
        type: "group",
        title: "🧩 Rewrite「重写」",
        description: "🧩 Rewrite「重写」\n仅对 rewrite_remote / filter_remote 生效。",
        items: [
          { type: "tags", key: "in", label: "✅ 保留分流/重写（in）",
            description: "按关键字保留 rewrite/filter。每行一个关键字，多行之间为“或”关系。",
            placeholder: "如：weibo" },
          { type: "tags", key: "out", label: "🗑 禁用分流/重写（out）",
            description: "按关键字禁用或删除 rewrite/filter。每行一个关键字，多行之间为“或”关系。",
            placeholder: "如：weibo" },
          { type: "tags", key: "inhn", label: "🌐 保留主机名（inhn）",
            description: "按关键字保留相关 hostname。每行一个关键字，多行之间为“或”关系。",
            placeholder: "如：weibo" },
          { type: "tags", key: "outhn", label: "🚫 删除主机名（outhn）",
            description: "按关键字删除相关 hostname。每行一个关键字，多行之间为“或”关系。",
