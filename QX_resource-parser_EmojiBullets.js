    ...
    sections: [
      {
        ...
        items: [
          { type: "switch", key: "UA", label: "UA 替换：用 Shadowrocket UA 重新获取订阅",
            onValue: "1", offValue: "",
            description: "📡 订阅返回空白、Unsupported Media Type、HTML 页面或乱码时打开。\n🔸 正常订阅可以不打开。" },
      ...
      },
      {
        ...
        items : [
          { type: "text", key: "via", label: "via-interface（规则订阅）",
            description: "🔗 给分流规则添加 via-interface。\n🔗 填写 0 表示 via-interface=%TUN%。\n🔗 设置代理链时，规则订阅通常需要这个参数。",
            placeholder: "0" },
          { type: "text", key: "relay", label: "Relay 目标策略（节点订阅转规则）",
            description: "🔐 把落地节点订阅转换成 ip/host 规则，并指向这里填写的中转策略组/节点名。\n🔸 使用时请关闭该订阅首页里的策略偏好设置。",
      ...
      },
      {
        ...
        items: [
          { type: "select", key: "ntf", label: "解析通知（ntf）",
            ...
            description: "🗑️ 规则类资源默认会提示解析结果，防止误删。" },
          { type: "select", key: "type", label: "强制资源类型（type）",
          ...
          { type: "switch", key: "cdn", label: "GitHub CDN 加速",
            onValue: "1", offValue: "",
            description: "🌐 把 GitHub raw 脚本地址转换为 jsDelivr CDN 地址。\n🔸 部分网络下加载更稳定。" },
          { type: "select", key: "dst", label: "转换目标（dst）",
            description: "🔸 强制把 module/rule-set 转成 rewrite 或 filter。\n⚙️ 一般保持默认；\n🔸 只有资源识别不符合预期时再改。",
            options: [
              { label: "默认：自动判断", value: "" },
          ...
          { type: "select", key: "fcr", label: "网络接口指定（fcr）",
            description: "🔸 给规则添加 force-cellular、multi-interface 或 multi-interface-balance。\n⚙️ 一般用户保持默认。",
            options: [
              { label: "默认", value: "" },
    ...
    sections: [
      {
        ...
        items: [
          { type: "tags", key: "in", label: "保留重写规则（in）",
            description: "🎯 只保留包含指定关键词的 rewrite/filter 规则。\n🔸 多行之间是“或”。",
            placeholder: "weibo\nyoutube" },
          { type: "tags", key: "out", label: "删除重写规则（out）",
            description: "🗑️ 删除包含指定关键词的 rewrite/filter 规则。\n💡 常用于禁用某个脚本，例如 tb_price.js、wb_ad.js。",
            placeholder: "tb_price.js\nwb_ad.js" },
          { type: "tags", key: "inhn", label: "保留 Hostname（inhn）",
            description: "🎯 只保留匹配关键词的 hostname。\n🔸 通常和 in 配合使用。",
            placeholder: "weibo" },
          { type: "tags", key: "outhn", label: "删除 Hostname（outhn）",
            description: "🗑️ 删除匹配关键词的 hostname。\n🗑️ 禁用某些重写脚本时，建议同时删除对应 hostname。",
            placeholder: "taobao\nweibo" },
          { type: "text", key: "regex", label: "正则保留（regex）",
            description: "🎯 正则匹配 rewrite/filter/hostname 完整内容，匹配成功才保留。\n⚠️ 慎用。",
            placeholder: "weibo|taobao" },
          { type: "text", key: "regout", label: "正则删除（regout）",
            description: "🗑️ 正则匹配 rewrite/filter/hostname 完整内容，匹配成功就删除。\n⚠️ 慎用，写错可能删空。",
            placeholder: "ads|tracking" },
      ...
      },
      {
        ...
        items: [
          { type: "select", key: "ntf", label: "解析通知（ntf）",
            ...
            description: "🗑️ 重写类资源默认会提示解析结果，防止误删。" },
          { type: "select", key: "type", label: "强制资源类型（type）",
