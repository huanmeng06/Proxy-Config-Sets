    ...
    sections: [
      {
        type: "group",
        title: "🚀 快速上手",
        description: "新手只需要看这一组。一般推荐：UA=1 解决获取异常；qxf=1 启用自动清洗；info=1 显示流量。",
        items: [
          { type: "switch", key: "UA", label: "UA 替换：用 Shadowrocket UA 重新获取订阅",
            onValue: "1", offValue: "",
            description: "订阅返回空白、Unsupported Media Type、HTML 页面或乱码时打开。正常订阅可以不打开。" },
          { type: "switch", key: "qxf", label: "一键启用 QX 自定义增强",
            onValue: "1", offValue: "",
            description: "新手推荐打开。等价于 clean=1&cleantag=1&autoe=1&airport=1&rsort=1。只整理节点名和排序，不修改服务器地址、端口、密码。" },
          { type: "switch", key: "info", label: "显示订阅流量/到期时间",
            onValue: "1", offValue: "",
            description: "机场提供流量信息时，会用通知显示已用流量、剩余流量、到期时间。" }
        ]
      },
      {
        type: "group",
        title: "🎲 筛选与排序",
        description: "先用 in/out 做普通关键词筛选；只有需要匹配协议、端口、加密方式等完整内容时，再用 regex/regout。",
        items: [
          { type: "tags", key: "in", label: "保留节点（in）",
            description: "只保留节点名包含指定关键词的节点。多行之间是“或”；同一行用 . 表示“与”。例：一行填 香港，另一行填 日本 = 保留香港或日本；填 香港.IPLC = 同时包含香港和 IPLC。",
            placeholder: "香港\n日本\n香港.IPLC" },
          { type: "tags", key: "out", label: "删除节点（out）",
            description: "删除节点名包含指定关键词的节点。适合删试用、倍率、官网、剩余流量、过期提示等。多行之间是“或”；同一行用 . 表示“与”。",
            placeholder: "倍率\n剩余流量\n官网" },
          { type: "text", key: "regex", label: "正则保留（regex）",
            description: "正则匹配节点完整内容，匹配成功才保留。可匹配协议、端口、加密、host 等字段。不会写正则时优先用 in。",
            placeholder: "iplc|iepl" },
          { type: "text", key: "regout", label: "正则删除（regout）",
            description: "正则匹配节点完整内容，匹配成功就删除。适合批量删某些协议、端口、倍率或关键词。慎用，写错可能删空订阅。",
            placeholder: "expire|traffic|剩余流量" },
          { type: "text", key: "sort", label: "节点排序（sort）",
            description: "1=按名称正序；-1=倒序；x=随机；也可写自定义顺序。> 表示这些关键词排前面，< 表示这些关键词排后面。",
            placeholder: "🇭🇰>🇯🇵>🇸🇬>🇨🇳>🇺🇸" }
        ]
      },
      {
        ...
        description: "qxf=1 会一次打开本组全部功能。若只想要其中一部分，可以单独打开 clean、cleantag、autoe、airport、rsort。台湾相关关键词统一显示 🇨🇳。",
        items: [
          { type: "switch", key: "clean", label: "删除说明/流量/官网/群组等伪节点",
            onValue: "1", offValue: "",
            description: "自动删除不像真实节点的条目，例如剩余流量、到期时间、官网、群组、更新提示、分割线等；会尽量保留防失联、备用、UDP、TCP 这类可能有用的节点。" },
          { type: "switch", key: "cleantag", label: "清理节点名特殊符号",
            onValue: "1", offValue: "",
            description: "删除节点名里的多余装饰符号、异常字符和杂乱空格，只保留中文、英文、数字、空格以及 - . _ () [] | 等常用符号。" },
          { type: "switch", key: "autoe", label: "自动识别地区 Emoji",
            onValue: "1", offValue: "",
            description: "根据节点名里的国家/地区关键词自动添加旗帜。中国台湾相关节点统一使用 🇨🇳。如果你还开了原版 emoji=1，建议先测试是否重复。" },
          { type: "switch", key: "airport", label: "插入机场名前缀",
            onValue: "1", offValue: "",
            description: "把 QX 资源名称作为 [机场名] 插入节点名。若节点名前已有 Emoji，则放在 Emoji 后面。适合同时导入多个机场时区分来源。" },
          { type: "switch", key: "rsort", label: "按地区顺序排序",
            onValue: "1", offValue: "",
            description: "按常用地区顺序排列：香港、日本、新加坡、中国、美国，其他地区放后面。中国台湾相关节点归入中国。" }
        ]
      },
      {
        type: "group",
        title: "⛰️ 节点参数调整",
        description: "这些参数会改节点配置字段。不了解含义时不要全开；多数情况下只需要 UDP、TFO、证书验证、Check URL。",
        items: [
          { type: "select", key: "emoji", label: "原版 Emoji 旗帜处理",
            description: "原作者内置 Emoji 处理。添加/删除节点名中的地区旗帜。若已开启 autoe，建议谨慎同时开启，避免重复处理。",
            options: _emojiOptions() },
          { type: "text", key: "replace", label: "正则替换节点完整内容（replace）",
            description: "高级功能。格式：正则@替换内容，多组用 + 连接。它会匹配节点完整内容，可改节点名，也可能改协议字段；写错会导致节点不可用。",
            placeholder: "旧字段@新字段" },
          { type: "switch", key: "udp", label: "UDP Relay",
            onValue: "1", offValue: "-1",
            description: "打开=强制 udp-relay=true；关闭=强制 udp-relay=false。游戏、语音、部分代理链可能需要 UDP。" },
          { type: "switch", key: "tfo", label: "Fast Open",
            onValue: "1", offValue: "-1",
            description: "打开=fast-open=true；关闭=fast-open=false。是否有效取决于系统、网络和节点服务端支持。" },
          { type: "switch", key: "cert", label: "TLS 证书验证",
            onValue: "1", offValue: "-1",
            description: "打开=校验证书；关闭=不校验证书。默认通常关闭。安全性优先可打开；部分自签或异常证书节点打开后可能不可用。" },
          { type: "switch", key: "uot", label: "UDP over TCP（仅 SS/SSR）",
            onValue: "1", offValue: "",
            description: "给 SS/SSR 节点添加 udp-over-tcp=true。只在你明确需要 UDP over TCP 时开启。" },
          { type: "switch", key: "aead", label: "VMess AEAD",
            onValue: "", offValue: "-1",
            description: "关闭 VMess AEAD。只有旧 VMess 节点兼容性异常时才考虑关闭。" },
          { type: "text", key: "alpn", label: "ALPN",
            description: "给 over-tls 类型节点指定 ALPN，例如 h2 或 http/1.1。不了解服务端配置时留空。",
            placeholder: "h2" },
          { type: "text", key: "host", label: "Host / SNI / 混淆 Host",
            description: "修改已有 host。若要新增 host，可在内容后加 ☠️。该项会影响连接，必须与机场配置一致。",
            placeholder: "example.com" },
          { type: "text", key: "checkurl", label: "测速 URL（server_check_url）",
            description: "指定节点可用性测试地址。建议使用稳定、轻量、可直连或可代理访问的 URL。",
      ...
      },
      {
        type: "group",
        title: "🎯 重命名",
        description: "rename 先按你写的规则改名；rrname 通常用于 Emoji 处理后再次改名。只想简单整理名称时，优先用 qxf 或 cleantag。",
        items: [
          { type: "text", key: "rename", label: "节点重命名（rename）",
            description: "格式：旧名@新名、前缀@、@后缀。多组用 + 连接。字段后加 ☠️ 表示删除该字段。例：香港@HK+@[1X]+倍率☠️。",
            placeholder: "香港@HK+@[1X]+倍率☠️" },
          { type: "text", key: "rrname", label: "Emoji 后重命名（rrname）",
            description: "在原版 emoji 处理之后再执行一次重命名。适合想让 Emoji 保持在最前，同时继续改后面的文字。",
            placeholder: "香港@HK" },
          { type: "select", key: "ptn", label: "英文字母样式（ptn）",
            description: "把节点名里的英文字母转换成花式字符。主要是美化用途；过度使用可能降低可读性。",
            options: _ptnOptions() },
          { type: "select", key: "npt", label: "数字样式（npt）",
            description: "把节点名里的数字转换成花式数字。主要是美化用途；过度使用可能降低可读性。",
            options: _nptOptions() }
        ]
      },
      {
        type: "group",
        title: "🧪 其他与调试",
        description: "解析异常、想看结果、或需要手动指定资源类型时再使用。",
        items: [
          { type: "select", key: "ntf", label: "解析通知（ntf）",
            ...
            description: "控制解析器通知。节点订阅默认偏安静；rewrite/filter 默认会提示，防止误删规则。" },
          { type: "text", key: "flow", label: "手动流量参数（flow）",
            description: "机场没有返回流量信息时可手动写。格式：到期日期:总流量GB:已用GB，例如 2026-12-31:1000:54。",
            placeholder: "2026-12-31:1000:54" },
          { type: "select", key: "type", label: "强制资源类型（type）",
            description: "自动识别失败时使用。节点订阅选 Nodes；分流规则选 Rule；模块选 Module；domain-set/list 按资源实际类型选择。",
            options: [
              { label: "自动识别", value: "" },
    ...
    sections: [
      {
        type: "group",
        title: "🔀 Filter 分流规则",
        description: "用于 filter_remote、rule-set、list 等分流规则资源。常用：policy 指定策略组；in/out 过滤规则；cdn 转换 GitHub 链接。",
        items: [
          { type: "tags", key: "in", label: "保留分流规则（in）",
            description: "只保留包含指定关键词的分流规则。多行之间是“或”。例如填 weibo，只保留含 weibo 的规则。",
            placeholder: "weibo\ntelegram" },
          { type: "tags", key: "out", label: "删除分流规则（out）",
            description: "删除包含指定关键词的分流规则。适合从远程规则里排除某些域名、APP 或脚本相关规则。",
            placeholder: "ads\ntracking" },
          { type: "text", key: "policy", label: "统一策略组（policy）",
            description: "把生成的分流规则统一指向这个策略组。默认是 Shawn。常见写法：PROXY、DIRECT、Reject、你的节点选择组名。",
            placeholder: "PROXY" },
          { type: "text", key: "pset", label: "按关键词指定不同策略组（pset）",
            description: "格式：正则1@策略组1+正则2@策略组2。适合一份规则里不同关键词走不同策略。",
            placeholder: "Google@PROXY+Apple@DIRECT" },
          { type: "text", key: "regex", label: "正则保留（regex）",
            description: "正则匹配规则完整内容，匹配成功才保留。不会正则时优先用 in。",
            placeholder: "Google|Telegram" },
          { type: "text", key: "regout", label: "正则删除（regout）",
            description: "正则匹配规则完整内容，匹配成功就删除。慎用，写错可能删空规则。",
            placeholder: "ads|tracking" },
          { type: "text", key: "replace", label: "正则替换规则内容（replace）",
            description: "格式：正则@替换内容。可批量修改策略名、域名、规则类型等；高级功能，建议先备份。",
            placeholder: "旧策略@新策略" }
        ]
      },
      {
        type : "group",
        title: "🔗 代理链 Relay",
        description: "代理链场景使用。一般规则订阅用 via，节点订阅转规则用 relay；两者不要在同一条引用里同时设置。",
        items : [
          { type: "text", key: "via", label: "via-interface（规则订阅）",
            description: "给分流规则添加 via-interface。填写 0 表示 via-interface=%TUN%。设置代理链时，规则订阅通常需要这个参数。",
            placeholder: "0" },
          { type: "text", key: "relay", label: "Relay 目标策略（节点订阅转规则）",
            description: "把落地节点订阅转换成 ip/host 规则，并指向这里填写的中转策略组/节点名。使用时请关闭该订阅首页里的策略偏好设置。",
      ...
      },
      {
        ...
        items: [
          { type: "select", key: "ntf", label: "解析通知（ntf）",
            ...
            description: "规则类资源默认会提示解析结果，防止误删。" },
          { type: "select", key: "type", label: "强制资源类型（type）",
            description: "自动识别失败时使用。rule/list/domain-set/module 按资源真实类型选择。",
            options: [
              { label: "自动识别", value: "" },
              { label: "Nodes 节点订阅", value: "nodes" },
              { label: "Rule 分流规则", value: "rule" },
          ...
          { type: "switch", key: "cdn", label: "GitHub CDN 加速",
            onValue: "1", offValue: "",
            description: "把 GitHub raw 脚本地址转换为 jsDelivr CDN 地址。部分网络下加载更稳定。" },
          { type: "select", key: "dst", label: "转换目标（dst）",
            description: "强制把 module/rule-set 转成 rewrite 或 filter。一般保持默认；只有资源识别不符合预期时再改。",
            options: [
              { label: "默认：自动判断", value: "" },
          ...
          { type: "select", key: "fcr", label: "网络接口指定（fcr）",
            description: "给规则添加 force-cellular、multi-interface 或 multi-interface-balance。一般用户保持默认。",
            options: [
              { label: "默认", value: "" },
    ...
    sections: [
      {
        type: "group",
        title: "🧱 Rewrite 重写规则",
        description: "用于 rewrite_remote 和模块中的 rewrite/hostname。常用：out 删除某些脚本，outhn 删除对应 hostname，cdn 加速 GitHub 脚本。",
        items: [
          { type: "tags", key: "in", label: "保留重写规则（in）",
            description: "只保留包含指定关键词的 rewrite/filter 规则。多行之间是“或”。",
            placeholder: "weibo\nyoutube" },
          { type: "tags", key: "out", label: "删除重写规则（out）",
            description: "删除包含指定关键词的 rewrite/filter 规则。常用于禁用某个脚本，例如 tb_price.js、wb_ad.js。",
            placeholder: "tb_price.js\nwb_ad.js" },
          { type: "tags", key: "inhn", label: "保留 Hostname（inhn）",
            description: "只保留匹配关键词的 hostname。通常和 in 配合使用。",
            placeholder: "weibo" },
          { type: "tags", key: "outhn", label: "删除 Hostname（outhn）",
            description: "删除匹配关键词的 hostname。禁用某些重写脚本时，建议同时删除对应 hostname。",
            placeholder: "taobao\nweibo" },
          { type: "text", key: "regex", label: "正则保留（regex）",
            description: "正则匹配 rewrite/filter/hostname 完整内容，匹配成功才保留。慎用。",
            placeholder: "weibo|taobao" },
          { type: "text", key: "regout", label: "正则删除（regout）",
            description: "正则匹配 rewrite/filter/hostname 完整内容，匹配成功就删除。慎用，写错可能删空。",
            placeholder: "ads|tracking" },
          { type: "text", key: "replace", label: "正则替换（replace）",
            description: "格式：正则@替换内容。可把脚本 URL、参数或策略字段替换成你指定的内容。",
            placeholder: "旧脚本@新脚本" }
        ]
      },
      {
        ...
        items: [
          { type: "select", key: "ntf", label: "解析通知（ntf）",
            ...
            description: "重写类资源默认会提示解析结果，防止误删。" },
          { type: "select", key: "type", label: "强制资源类型（type）",
            description: "自动识别失败时使用。rewrite/module/rule/list/domain-set 按资源真实类型选择。",
            options: [
              { label: "自动识别", value: "" },
              { label: "Nodes 节点订阅", value: "nodes" },
              { label: "Rule 分流规则", value: "rule" },
          ...
          { type: "switch", key: "cdn", label: "GitHub CDN 加速",
            ...
            description: "把 GitHub raw 脚本地址转换为 jsDelivr CDN 地址。部分网络下加载更稳定。" },
          { type: "select", key: "dst", label: "转换目标（dst）",
            description: "强制把 module/rule-set 转成 rewrite 或 filter。一般保持默认。",
            options: [
              { label: "默认：自动判断", value: "" },
              { label: "转为 Rewrite", value: "rewrite" },
              { label: "转为 Filter", value: "filter" }
